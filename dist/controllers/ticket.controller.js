...
const CUSTOMER_TICKET_LIST_SELECT = TICKET_LIST_FIELDS.filter((field) => field !== "inverter.warrantyEnd" && !field.startsWith("statusHistory")).join(" ");
...
function isAdminTierRole(roleRaw) {
...
function parseRaiseDate(raw, previous) {
    ...
    const next = new Date(Date.UTC(year, month - 1, day, prev ? prev.getUTCHours() : 0, prev ? prev.getUTCMinutes() : 0, prev ? prev.getUTCSeconds() : 0, prev ? prev.getUTCMilliseconds() : 0));
...
exports.updateTicket = (0, error_middleware_1.asyncHandler)(async (req, res) => {
    ...
    if (Object.prototype.hasOwnProperty.call(body, 'createdAt')) {
        ...
        const nextCreatedAt = parseRaiseDate(body.createdAt, ticket.createdAt);
        if (!nextCreatedAt) {
            return res.status(400).json({
                success: false,
        ...
        if (isOnsite) {
            ...
            if (prevFlow === "UNDER_REPAIRED" && nextFlow === "UNDER_DISPATCH") {
                ...
                if (!["REPAIRABLE", "NOT_REPAIRABLE"].includes(final)) {
                    return res.status(400).json({
                        success: false,
                        message: "Cannot move to UNDER_DISPATCH until the engineer finalizes the job card as REPAIRABLE or NOT REPAIRABLE (SCRAP).",
                    });
                }
            }
    ...
    if (ticket.status && ticket.status !== prevStatus) {
        void safeCreateNotification({
            ...
            targetUsers: [
                ...
                ticket?.salesAssignee?._id || ticket?.salesAssignee,
                ...(Array.isArray(ticket?.assignedTo) ? ticket.assignedTo : []),
            ],
        });
    }
...
exports.updateTicketJobCard = (0, error_middleware_1.asyncHandler)(async (req, res) => {
    ...
    if (canSetFinal) {
        ...
        const prevFinal = String(jobcard?.engineerFinalStatus || "").toUpperCase().trim();
