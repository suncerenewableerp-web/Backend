...
const fmtDate = (d) => d ? `${d.getUTCFullYear()}-${String(d.getUTCMonth() + 1).padStart(2, "0")}-${String(d.getUTCDate()).padStart(2, "0")}` : "";
...
function makeTicketIdFactory(existingIds) {
...
async function main() {
    ...
    for (const [field, header] of Object.entries(resolver))
        console.log(`   ${field.padEnd(15)} ← "${header}"`);
    ...
    for (const row of dataRows) {
        ...
        if (d)
            importYears.add(d.getUTCFullYear());
    }
    console.log(importYears.size
    ...
    const cursor = Ticket_model_1.default.find(scanFilter, { ticketId: 1, "inverter.serialNo": 1, "inverter.model": 1, "inverter.capacity": 1, "customer.company": 1, createdAt: 1 }).lean().cursor();
    for await (const t of cursor) {
        if (t?.ticketId)
            existingIds.add(String(t.ticketId));
    ...
    const payloads = [];
    const bumpYear = (year, field) => {
        var _a;
        const key = year == null ? "NO_DATE" : String(year);
        (_a = report.byYear)[key] || (_a[key] = { candidates: 0, imported: 0, skippedDuplicate: 0, skippedOutsideTargetYears: 0, skippedAfterYearCutoff: 0, forcedClosed: 0 });
    ...
    dataRows.forEach((row, i) => {
        ...
        try {
            ...
            const forceClosed = ticketYear != null && (closeYears.has(ticketYear) ||
            ...
            if (rawStatus)
                noteParts.push(`Stage: ${rawStatus}`);
            if (forceClosed)
                noteParts.push(`Forced CLOSED for ${ticketYear} import rule`);
            if (lrNo)
                noteParts.push(`LR No: ${lrNo}`);
            if (dateRepaired)
                noteParts.push(`Repaired: ${fmtDate(dateRepaired)}`);
            if (dateDispatched)
                noteParts.push(`Dispatched: ${fmtDate(dateDispatched)}`);
    ...
    if (report.skippedOutsideTargetYears)
        console.log(`⏭️  Skipped ${report.skippedOutsideTargetYears} row(s) outside target years`);
    if (report.skippedAfterYearCutoff)
        console.log(`⏭️  Skipped ${report.skippedAfterYearCutoff} row(s) after year cutoff`);
    if (report.forcedClosed)
        console.log(`🔒 Forced CLOSED on ${report.forcedClosed} ticket(s)`);
    ...
    for (const [year, counts] of Object.entries(report.byYear).sort(([a], [b]) => a.localeCompare(b))) {
        console.log(`   ${year}: candidates=${counts.candidates} new=${counts.imported} duplicate=${counts.skippedDuplicate} outsideYears=${counts.skippedOutsideTargetYears} cutoff=${counts.skippedAfterYearCutoff} forcedClosed=${counts.forcedClosed}`);
    }
    if (commit && payloads.length) {
        ...
        try {
            for (let i = 0; i < payloads.length; i += BATCH_SIZE) {
                ...
                const opts = { ordered: true, timestamps: false };
                if (useTxn && session)
                    opts.session = session;
                await Ticket_model_1.default.insertMany(batch, opts);
