...
type CrudPermission = { view: boolean; create: boolean; edit: boolean; delete: boolean };
type PermissionMatrix = Record<string, CrudPermission>;

const none = (): CrudPermission => ({ view: false, create: false, edit: false, delete: false });
const full = (): CrudPermission => ({ view: true, create: true, edit: true, delete: true });

function basePermissions(): PermissionMatrix {
  return Object.fromEntries(MODULES.map((m) => [m, none()])) as PermissionMatrix;
}

function withOverrides(overrides: Record<string, Partial<CrudPermission>>) {
...
const roles = await Role.insertMany([
  ...
  const seedDemoTickets = process.env.SEED_DEMO_TICKETS === "true";
  ...
  if (seedDemoTickets) {
    ...
    function pickFinalStatus(monthIndexFromOldest, itemIndexInMonth) {
      ...
      if (monthIndexFromOldest === 2) return ["CLOSED", "DISPATCHED", "UNDER_REPAIRED", "UNDER_REPAIRED"][itemIndexInMonth % 4];
      if (monthIndexFromOldest === 3) return ["DISPATCHED", "UNDER_REPAIRED", "IN_TRANSIT", "PICKUP_SCHEDULED"][itemIndexInMonth % 4];
      if (monthIndexFromOldest === 4) return ["UNDER_REPAIRED", "IN_TRANSIT", "PICKUP_SCHEDULED", "CREATED"][itemIndexInMonth % 4];
    ...
    console.log("🎫 Tickets: 0 (demo tickets skipped; set SEED_DEMO_TICKETS=true to include them)");
  }
